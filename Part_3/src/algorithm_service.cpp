...
SVP_NNIE_MODEL_WITH_FILE_S *AlgorithmService::get_model() {
    return &m_model;
}
...
int AlgorithmService::SDC_UnLoadModel(SVP_NNIE_MODEL_S *pstModel) {
    int nRet = -1;
    if (NULL != pstModel) {
        SDC_COMMON_HEAD_S head;
        struct iovec iov[2] = {
        ...
        if (nRet < 0) {
            DEBUG_LOG("Errin SDC_UnLoadModel:failed to unload nnie module!");
        }
    } else {
        DEBUG_LOG("Err in SDC_UnLoadModel:module pointer is NULL!");
    }
